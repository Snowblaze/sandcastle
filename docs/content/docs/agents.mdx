...
The default agent provider. Uses OpenAI Codex as the AI coding agent inside the sandbox.

Required environment variables:

| Variable | Description |
| --- | --- |
| `OPENAI_API_KEY` | OpenAI API key. Optional when `CODEX_AUTH_JSON_B64` is set. |
| `CODEX_AUTH_JSON_B64` | Base64-encoded `~/.codex/auth.json` for Codex CLI auth inside Docker. Optional when `OPENAI_API_KEY` is set. |
